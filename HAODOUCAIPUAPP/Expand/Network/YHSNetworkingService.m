...
static NetResponseType sg_responseType = kNetResponseTypeJSON;
static NetRequestType  sg_requestType  = kNetRequestTypeJSON;

@implementation YHSNetworkingService
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/NetNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"NetNetworking clear caches error: %@", error);
        } else {
            NSLog(@"NetNetworking clear caches ok");
        }
    }
}
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(NetURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[NetURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (void)cancelRequestWithURL:(NSString *)url {
    ...
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(NetURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[NetURLSessionTask class]] && [task.currentRequest.URL.absoluteString hasSuffix:url]) {
                [task cancel];
...
+ (NetURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(NetDownloadProgress)progress
                               success:(NetResponseSuccess)success
                                  fail:(NetResponseFail)fail {
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            NetLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试 Encode URL");
            return nil;
        }
    } else {
        ...
        if (absouluteURL == nil) {
            NetLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    }
    ...
    NetURLSessionTask *session = nil;
    
    if (httpMethod == 1) { // GET
        if (sg_cacheGet && !refreshCache) { // 获取缓存
            id response = [YHSNetworkingService cahceResponseWithURL:absolute
                                                  parameters:params];
            if (response) {
                if (success) {
...
+ (NetURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        NetLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        NetLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (NetURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    NetURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (NetURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                NetLog(@"Download success for url: %@", [self absoluteUrlWithPath:url]);
            }
        } else {
...
+ (AFHTTPSessionManager *)manager {
    ...
    switch (sg_requestType) {
        case kNetRequestTypeJSON: {
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            break;
        }
        case kNetRequestTypePlainText: {
    ...
    switch (sg_responseType) {
        case kNetResponseTypeJSON: {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];
            break;
        }
        case kNetResponseTypeXML: {
            manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
            break;
        }
        case kNetResponseTypeData: {
...
+ (void)successResponse:(id)responseData callback:(NetResponseSuccess)success {
    if (success) {
        success([self tryToParseData:responseData]);
    }
}

+ (NSString *)net_URLEncode:(NSString *)url {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        if (data) {
            cacheData = data;
            NetLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                NetLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                NetLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
...
+ (void)handleCallbackWithError:(NSError *)error fail:(NetResponseFail)fail {
    if ([error code] == NSURLErrorCancelled) {
