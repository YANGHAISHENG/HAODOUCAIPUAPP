...
- (void)applyDefaultsToSelfDuringInitializationWithFrame:(CGRect)frame pages:(NSArray *)pagesArray {
    ...
    _titleViewY = 20.f;
    _pageControlY = 50.f;
    _skipButtonY = YHS_EMPTY_PROPERTY;
...
- (YHSIntroPage *)pageForIndex:(NSUInteger)idx {
    if(idx >= _pages.count) {
        return nil;
    }
    
    return (YHSIntroPage *)_pages[idx];
}
...
- (void)checkIndexForScrollView:(YHSRestrictedScrollView *)scrollView {
...
- (void)notifyDelegateWithPreviousPage:(NSUInteger)previousPageIndex andCurrentPage:(NSUInteger)currentPageIndex {
    if(currentPageIndex!=_currentPageIndex && currentPageIndex < _pages.count) {
        YHSIntroPage* previousPage = _pages[previousPageIndex];
        YHSIntroPage* currentPage = _pages[currentPageIndex];
...
- (YHSRestrictedScrollView *)scrollView {
    if (!_scrollView) {
        _scrollView = [[YHSRestrictedScrollView alloc] initWithFrame:self.bounds];
...
- (UIView *)viewForPage:(YHSIntroPage *)page atXIndex:(CGFloat)xIndex {
...
- (void)buildFooterView {
    ...
    if (self.skipButton && !self.skipButton.hidden) {
        ...
        if(self.skipButtonY == YHS_EMPTY_PROPERTY) {
...
- (void)scrollViewWillBeginDragging:(YHSRestrictedScrollView *)scrollView {
...
- (void)scrollViewDidEndDecelerating:(YHSRestrictedScrollView *)scrollView {
...
- (void)scrollViewDidEndScrollingAnimation:(YHSRestrictedScrollView *)scrollView {
    [self checkIndexForScrollView:scrollView];
}

- (void)scrollViewDidScroll:(YHSRestrictedScrollView *)scrollView {
...
- (void)showInView:(UIView *)view animateDuration:(CGFloat)duration withInitialPageIndex:(NSUInteger)initialPageIndex {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        YHSIntroPage *currentPage = _pages[self.currentPageIndex];
        if(currentPage.onPageDidAppear) currentPage.onPageDidAppear();
