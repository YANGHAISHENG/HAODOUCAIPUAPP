//
//  YHSCookBookQualityReadTableViewCell.m
//  HAODOUCAIPUAPP
//
...
NSString * const CELL_IDENTIFIER_QUALITY_READ = @"YHSCookBookQualityReadTableViewCellID";

@interface YHSCookBookQualityReadTableViewCell ()
...
@implementation YHSCookBookQualityReadTableViewCell
 ...
 *  设置控件属性
 */
- (void)setModel:(YHSCookBookQualityReadModel *)model
{
    _model = model;
    
    // 内容是否为空
    if (!_model || !model.readList.count) {
        return;
    }
    ...
    for (int i = 0; i < _model.readList.count; i ++) {
        
        YHSCookBookQualityReadElemModel *elem = _model.readList[i];
        ...
        UIView *containerView = ({
            ...
            UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(pressReadViewArea:)];
...
- (void)pressReadViewArea:(UITapGestureRecognizer *)gesture
{
    
    UIView *view = (UIView*)gesture.view;
    NSUInteger index = view.tag;

    YHSCookBookQualityReadElemModel *model = self.model.readList[index];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickElementOfCellWithQualityReadModel:)]) {
        [self.delegate didClickElementOfCellWithQualityReadModel:model];
    }
