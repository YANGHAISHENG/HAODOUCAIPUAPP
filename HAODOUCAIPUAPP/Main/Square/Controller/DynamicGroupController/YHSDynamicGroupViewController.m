//
//  YHSDynamicGroupViewController.m
//  HAODOUCAIPUAPP
//
...
@interface YHSDynamicGroupViewController () <UITableViewDelegate, UITableViewDataSource, YHSDynamicGroupTableViewCellDelegate>
...
@implementation YHSDynamicGroupViewController
...
- (void)createUITable
{
    ...
    // 创建表格
    {
        ...
        [self.tableView registerClass:[YHSDynamicGroupTableViewCell class] forCellReuseIdentifier:CELL_IDENTIFIER_DYNAMIC_GROUP];
    }
...
- (void)loadDataThen:(void (^)(BOOL success, NSUInteger count))then andWritingLoading:(BOOL)showWritingLoading {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // 请求地址与参数
        NSString *url = [YHSSquareDataUtil getDynamicGroupRequestURLString];
        NSMutableDictionary *params = [YHSSquareDataUtil getDynamicGroupRequestParams];
        ...
        [manager POST:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            [list enumerateObjectsUsingBlock:^(NSDictionary *  _Nonnull dict, NSUInteger idx, BOOL * _Nonnull stop) {
                YHSDynamicGroupModel *model = [YHSDynamicGroupModel mj_objectWithKeyValues:dict];
...
{
    YHSDynamicGroupTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CELL_IDENTIFIER_DYNAMIC_GROUP];
    if (!cell) {
        cell = [[YHSDynamicGroupTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CELL_IDENTIFIER_DYNAMIC_GROUP];
    }
    cell.delegate = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [self.tableView fd_heightForCellWithIdentifier:CELL_IDENTIFIER_DYNAMIC_GROUP cacheByIndexPath:indexPath configuration:^(YHSDynamicGroupTableViewCell *cell) {
